...
- **Local ONNX translation fallback** (`F-14`) — when the cloud LLM API fails (network errors, quota exhaustion, offline), the extension automatically falls back to a local ONNX small model (`onnx-community/Qwen2.5-0.5B-Instruct`, INT4 ONNX, ~350MB) for fully offline translation. **Architecture**: a new `LocalONNXTranslationProvider` adapter implements the `TranslationProvider` port, sending inference requests via Chrome Message Bus to the Service Worker, which forwards them to the Offscreen Document (with full DOM and WASM support) to execute ONNX Runtime Web inference. The ONNX Runtime WASM binaries are bundled into the extension (no external CDN dependency) and the model is fetched with `dtype: 'q4'` (INT4). **Primary-engine support**: the Options "Engine type" selector can choose "Local ONNX model (offline)" as the primary translation engine, enabling fully offline translation without any cloud API; on failure it still degrades to MT or original text. **Options UI**: a new "Local Fallback Model" section displays the model name (read-only), status badge (`Not downloaded` / `Downloading xx%` / `Ready` / `Download failed`), download progress bar (real-time byte percentage and speed), and "Download Model" / "Clear Cache" buttons. **Fallback chain**: `TranslationConfig.fallbackType` adds `'local-onnx'` option; `Orchestrator` prioritizes `local-onnx` as fallback (`local-onnx > mt > undefined`). If the model isn't downloaded, the provider throws an error with `local-onnx-not-downloaded` diagnostic, and the pipeline continues degrading to MT or original text.
