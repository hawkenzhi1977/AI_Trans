...
describe('FetchCaptionSource — R1 fetch 綁定 / R2 URL 絕對化 / R7 JSON 容錯', () => {
  ...
  it('[M2-22 補充] 非 watch 頁（currentVid 為空）→ 返回空並記錄診斷', async () => {
    ...
    const src = new FetchCaptionSource(document);
    const list = await src.fetchTrackList();
    // 應該返回空陣列（非 watch 頁無視頻可翻譯）。
    expect(list).toEqual([]);
    expect(src.getLastTrackDiagnostic()).toContain('not on watch page');
    window.history.replaceState({}, '', '/');
  });

  it('[M2-22 第三層] videoId 不匹配 + 播放器 API 不可用 + bridge 有 captured tracks → 使用 bridge tracks', async () => {
    // 當前頁面 URL 為視頻 B（v=bbb），但 ytInitialPlayerResponse 包含視頻 A（v=aaa）的數據。
    window.history.replaceState({}, '', '/watch?v=bbb');
    const stalePlayerResponse = JSON.stringify({
      videoDetails: { videoId: 'aaa' },
    ...
    setPlayerResponse(stalePlayerResponse);
    // 無播放器元素（模擬 content script 無法訪問 movie_player.getOption）。
    document.body.innerHTML = '';
    const script = document.createElement('script');
    script.id = 'ytInitialPlayerResponse';
    script.type = 'application/json';
    script.textContent = stalePlayerResponse;
    document.body.appendChild(script);
    // Mock captureProvider 返回 bridge 捕獲的軌道信息。
    const mockCaptureProvider = {
      getLatest: () => null,
      getCapturedTracks: (videoId: string) => {
        if (videoId === 'bbb') {
          return [{ lang: 'en', baseUrl: '/timedtext?v=bbb&lang=en', isAutoGenerated: false, videoId: 'bbb' }];
        }
        return [];
      },
    };
    const src = new FetchCaptionSource(document, globalThis.fetch, mockCaptureProvider);
    const list = await src.fetchTrackList();
    // 應該返回 bridge 捕獲的軌道（v=bbb），而非 stale DOM 數據（v=aaa）。
    expect(list).toHaveLength(1);
    expect(list[0].baseUrl).toContain('v=bbb');
    window.history.replaceState({}, '', '/');
  });
