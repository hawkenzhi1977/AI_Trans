...
> **該選哪個引擎？** — **雲端 API**（品質最佳、零本地資源、需網路）→ **Ollama/llama.cpp 本地服務**（品質好、原生速度、隱私、記憶體 <400MB）→ **本地 ONNX 大型**（完全離線、免費、~750MB，但比原生推理慢且質量較低）→ **本地 ONNX 中型**（極端硬體限制時使用、~200MB）→ **MT**（即時可用、零設定，但品質最低；適合當最後兜底）。**除非萬不得已，避免使用本地 ONNX 小型**——它在瀏覽器 WASM 環境下存在嚴重質量問題。實時字幕建議選快速、翻譯導向（MT）或小型 instruct 模型，避免大型通用模型；本地伺服器勿用 ASR 模型（它們走 `/v1/audio/transcriptions`，不是 `/chat/completions`）。

> **本地 ONNX — 時延與記憶體限制**：本地模型完全在瀏覽器內執行。首次使用需下載模型檔案；下載後**首次翻譯可能耗時 30–60 秒**（載入模型到記憶體）——在設定頁點**「預加載模型」**（或仰賴背景自動預熱），首響應延遲即可接近即時。翻譯以**每 3–5 段一個 chunk**（依檔位而定）處理，影片很長時完整翻譯需較久；單次翻譯會話硬性上限**10 分鐘**，以維持擴充穩定。記憶體佔用依檔位與後端而異：WASM 模式（預設）約 **~150–750MB JS 堆**（依模型而定）；有 WebGPU 時約 **~50–100MB JS 堆 + GPU VRAM**（權重移入 VRAM）。低記憶體機器建議改用雲端或本地 LLM 引擎。

> **本地 ONNX 模型檔位選擇指南**：擴充提供三檔模型以適應不同硬體配置：
> - **大型（Qwen2.5-0.5B，~750MB）** — **推薦默認**。翻譯質量最高，支援多語言，採用現代 Decoder-only 架構與 BPE 分詞器。適合 ≥8GB RAM 且有 WebGPU 支援的機器。
> - **中型（SmolLM2-360M，~200MB）** — 平衡質量與性能。適合無法運行大型模型的低配置機器。使用 SmolLM2 chat template。注意：中文語料佔比低於 Qwen，英→中翻譯質量可能略低。
> - **小型（MarianMT/opus-mt-en-zh，~150MB）** — **⚠️ 不推薦**。此模型在瀏覽器 WASM 環境下存在嚴重質量問題：經常產生退化輸出（重複循環如「我希望我希望...」或亂碼字符），根因為 q8 量化精度損失 + WASM 後端限制 + 慢版分詞器。即使有檢測與回退機制，翻譯質量仍不可靠。僅適合極端硬體限制且無其他選項的情況。
>
> **檔位切換行為**：在設定中切換模型檔位時，擴充自動清除舊檔位的快取檔案與記憶體資源，並刷新 UI 顯示新檔位的下載狀態。若新檔位尚未下載，需重新下載。
...
> - **量化精度損失**：WASM 下的 INT8/Q8 量化比原生 INT4/INT8 損失更多精度，特別是 MarianMT 等 Seq2Seq 模型。
...
>
> **引擎優先級推薦**：**雲端 API**（質量最佳、零本地資源）→ **Ollama/llama.cpp 本地服務**（質量好、原生速度、隱私）→ **本地 ONNX 大型**（離線、但較慢且質量較低）→ **本地 ONNX 中型**（極端限制时使用）→ **MT**（即時但質量最低）。**除非萬不得已，避免使用本地 ONNX 小型**。
