...
> 最後更新：2026-09-17（**M2-71：ASR 默認改多語言模型（無字幕非英語視頻「僅首行英文、無中文」根因修復）**——§ASRConfig 補 M2-71：`config.ts` `PROFILE_DEFAULTS` 三檔（streaming/balanced/quality）與 `DEFAULT_CONFIG.asr.modelTier` 皆改為 `-multi` 變體（tiny-multi/base-multi/small-multi）。根因：默認 `base`→`Xenova/whisper-base.en`（English-only）無法轉錄非英語音頻；多語言變體 `generation_config.json` 自帶 `is_multilingual: true`+`lang_to_id`/`task_to_id`，`runAsrInference` 傳入 language/task 即可正確轉錄（無需顯式傳 is_multilingual）。English-only（`.en`）變體仍供用戶手動選擇。先前：**M2-70：實時 ASR 字幕寬限期 + hintLang 接線 + 累積窗口縮短（「ASR 字幕不顯示」根因修復）**——§7.5 補 M2-70：①`overlay-renderer.ts` `draw()` 新增 `ASR_GRACE_PERIOD_MS = 5000`——嚴格窗口（`currentTime >= c.start && currentTime < c.end`）未命中時，對 `origin === 'realtime-asr' | 'lookahead-asr'` 的 cue 在 `currentTime - c.end <= 5s` 內仍顯示（取最接近的）；native cue 不享有寬限期。②`RenderableCue`（subtitle-renderer.ts）新增 `origin?: CaptionOrigin`，content-script 從 `SubtitleSegment.origin` 填充，讓渲染層區分 ASR/native cue。③`realtime-asr-strategy.ts:291` `hintLang: undefined` → `ctx.audioLanguage`（orchestrator 已從 `platform.getAudioLanguage()` 傳入），Whisper 推理獲得語言提示。④`offscreen.ts` `AUDIO_ACCUMULATE_TARGET_MS` 默認 3000→2000（Options UI 2s selected）。先前：**M2-69：Offscreen 單例衝突健壯化（ASR warmup「Only a single offscreen document may be created」→ 中英文字幕皆不出現）**——§7.5 Offscreen Document 通信協議補 M2-69：`service-worker.ts` `ensureOffscreenDocument()` 的 `getContexts` 改為不帶 `documentUrls` 過濾（僅按 contextType 匹配，更寬鬆可靠）；`createDocument()` 被拒「single offscreen document」時重查 `getContexts`，確認已存在視為成功；`LocalWhisperASR.warmup()` 偵測 single offscreen 錯誤時發 `offscreen:ensure-created` + 重試一次（覆蓋 SW offscreenPort 指向已關閉舊 document 的 port 路徑失敗）。先前：**M2-67：重新授權 ASR 失敗修復 + non-speech token 去重正規化**——§7.5 tabCapture.getMediaStreamId 補 M2-67：SW `asr:get-stream-id` handler 調用 `getMediaStreamId` 前先經 offscreen-onnx port 發 `asr:release-stream`（best-effort），offscreen 執行 detach + stop tracks + mediaStream=null；`realtime-asr-strategy.ts` `normalizeForDedup()` 將 `[MUSIC]`/`(laughing)` 等 non-speech token 正規化為 `"ns"`，流式/非流式路徑去重比較改用正規化後文本。先前：**M2-66：實時 ASR 字幕不顯示修復（音頻累積 + 顯示窗口擴展 + Backpressure + ASR 去重）**——§7.5 補 M2-66：`offscreen.ts` 新增 `AUDIO_ACCUMULATE_TARGET_MS = 5000` + `audioAccumBuffer: Float32Array | null`（onaudioprocess 累積至 ~5s 才發送 audioChunk，解決 256ms 碎片 → Whisper 產出重複 `[MUSIC]` + 隊列爆炸）；`realtime-asr-strategy.ts` 新增 `MIN_DISPLAY_WINDOW_MS = 5000`（alignSegmentsToVideoTimeline 中 end = max(rawEnd, start + 5s)，確保推理延遲期間 cue 仍可被 overlay-renderer 渲染）+ `MAX_INFLIGHT_ASR = 2`（inflight ≥ MAX 時跳過新 chunk，try/finally 遞減）+ `DEDUP_CONSECUTIVE_THRESHOLD = 3`（lastAsrText + consecutiveDuplicateCount，流式/非流式路徑翻譯前去重）。先前：**M2-65：local-onnx 電路斷開器 + RealtimeASRStrategy 累計 emit**——§7.5 補 M2-65：`local-onnx-translation.ts` 新增 `CIRCUIT_BREAKER_THRESHOLD = 3`（per-session 電路斷開器，連續 3 chunk echoed/degenerate/wrongLanguage 後跳過推理直接回退原文，成功重置計數，診斷碼 `local-onnx-circuit-breaker-opened`）；`realtime-asr-strategy.ts` 新增 `accumulatedSegments: Map<string, SubtitleSegment>`（按 segment id 去重/覆蓋，`emitAligned()` 改為合併入緩衝後 emit 全量排序列表，`stop()`/`inject()` 清空——解決 content-script 全量替換 `this.cues` 導致舊字幕丟失問題）。先前：**M2-39：三檔位模型支持 + 檔位切換清理舊快取 + 默認檔位改為 Large**：§7.5 補 M2-39——`LOCAL_TRANSLATION_MODELS` 擴展為三檔位（`small`/`medium`/`large`）；`medium: 'onnx-community/SmolLM2-360M-Instruct'`（~200MB，平衡質量與性能）；默認檔位從 `'small'` 改為 `'large'`（`composition.ts`/`options.ts`/`offscreen.ts` 全部 `?? 'large'`）；`setModelTier()` 改為 `async`，切換時新增 `clearCacheForModel(oldModelName)` 清除舊檔位 Cache API 快取；`hasModelInCache()` 修正為精確匹配 `currentModelName`（移除 `.onnx` 泛匹配）；消息處理器 `set-model-tier` 改為 `await` 後查詢真實狀態；新增 `buildPromptSmolLM2()` 函數（SmolLM2 chat template：`<|system|>...<|end|><|user|>...<|end|><|assistant|>`）；`runInference` 根據 `currentModelTier` 選擇 prompt 格式（medium → SmolLM2，large → ChatML）；`CHUNK_SIZE_MEDIUM = 4`；Options UI 新增 medium 選項 + Small model `⚠ 質量不穩定` 警告 + `MODEL_SIZES.medium`。）
> 前次更新：2026-08-27（**M2-38：Small model (MarianMT) 雙檔位支持 + 退化輸出檢測**：§7.5 補 M2-38——`LOCAL_TRANSLATION_MODELS` 常量定義 `small: 'Xenova/opus-mt-en-zh'`（MarianMT，150MB）與 `large: 'onnx-community/Qwen2.5-0.5B-Instruct'`（750MB）雙檔位；`LocalModelTier = 'small' | 'large'` 類型、`TranslationConfig.localModelTier` 配置項；`currentModelTier` + `setModelTier()` + `isSmallModel()` + `smallModelBroken` 檔位切換與 WASM 不相容標記；`runInference` 雙檔位推理路徑分化（Small: `pipeline('translation', { dtype: 'q8' })` + `max_new_tokens: 256`/`repetition_penalty: 1.2`/`no_repeat_ngram_size: 3`；Large: `pipeline('text-generation', { dtype: 'q4' })` + ChatML Prompt）；`calcUniqueNgramRatio(text, n)` 純函數計算 3-gram 唯一率，Small model 推理後檢測唯一率 <0.2 且長度 >100 時返回 `degenerate: true`；`CHUNK_SIZE_SMALL = 3` / `CHUNK_SIZE_LARGE = 5` 分塊策略；`translateChunk` 退化響應處理（回退原文 + `small-model-degenerate` 診斷 + 提示用戶切換 Large model，不自動切換）；`smallModelStats` 統計 merged/perfect/fallback chunks；消息協議擴展 `modelTier` 請求字段 + `degenerate` 響應字段。）
> 前次更新：2026-08-20（**M1-59 + M2-27：滑動窗口起點依引擎區分 + Options 模型狀態即時顯示**：§7.4/M1-57 滑動窗口統一 `+2s` 改為**依引擎區分**——本地 ONNX（主/兜底，配置偵測）窗口起點 `currentTime+5s`（`WINDOW_START_OFFSET_ONNX_MS`）、其餘引擎 `currentTime+0s`（`WINDOW_START_OFFSET_DEFAULT_MS`），新增純函數 `windowStartOffsetMs(translation)`、`getPrioritizedSegments(ct, offsetMs)` 參數化偏移；§7.5/M2-26 後補 **M2-27 Options 模型狀態即時顯示**——offscreen 狀態字段擴充（ONNX `loaded/loading/downloading?`、ASR `downloading?` + `asrDownloadInProgress`）、背景預熱完成 `broadcastToAll` 廣播、Options `applyModelStatus` 五態渲染 + onMessage 監聽即時刷新、`setup-dom` onMessage 監聽器 Set 基建。）
> 前次更新：2026-08-20（**M2-26：WebGPU 翻譯後端 + popup init watchdog + manifest commands 移除**：真實環境「本地 ONNX 翻譯模型載入後長期播放 popup 彈不出」——wasm 推理把 750MB 模型倉庫放進 JS heap（~500MB）撐爆與 popup 共享的 extension 渲染進程。§7.5 補 M2-26：`loadPipeline` webgpu-first（權重駐 VRAM、heap ~50→100MB）+ `webgpuFailed`/`preferWebGpu()` 回退 + `local-onnx-webgpu-fallback` 診斷 + `logJsHeapBreadcrumb`；popup init watchdog（3s slow / 10s timeout，port 'platform'）；manifest `commands` 塊移除（自訂 command 破壞 SW 註冊，Options 改經 popup 按鈕；**補充修復**：無 commands 鍵時 Chrome 裁剪 `chrome.commands` 命名空間，頂層死監聽令 SW 註冊失敗 status 15——同步刪除 onCommand 監聽）；SW 生命週期麵包屑（offscreen created + onStartup/onInstalled/onSuspend）。先前：**M2-25：Offscreen 空閒生命週期**（IdleTimeout + shutdownForIdle → offscreen:idle-close → closeDocument，`offscreen-close-failed` 診斷）。先前：**補充修復十六：local-onnx 流式漸進 + echo 診斷儀器**：真實環境「翻譯啟動但字幕完全空白」——`LocalONNXTranslationProvider.translateStream()` 非真流式（`await this.translate(req)` 全量 87 chunk 串行推理後才 emit 一次），而 `NativeCaptionStrategy` 僅首次 emit 發 `segments-ready` → 數分鐘無渲染。修復：抽取 `translateChunk()`（攜 Offscreen 判定的 echo 標記），`translateStream` 逐 chunk emit 累計全量（與 LLM 同語義）；`parseNumberedOutput` 返回 `parsedCount`、echo 診斷 cause 內嵌 raw `generated_text` 前 200 字符 + `parsed x/N` + 推理耗時、translate-result 響應新增 `echoed?: boolean`、provider 聚合 `local-onnx-echo-chunks` 診斷、新增 `local-onnx` 第九調試分類。純診斷 + 流式修復，不做任何降級行為改變。測試 +3 → **339**。先前：**補充修復十五：清快取後重新下載模型永遠失敗（世代失效 + 進度防護）**：`clearModelCache()` 原只刪快取、未重置共享載入狀態——`checkModelStatus()` 後台預熱建立「無進度回調」的 in-flight `loadPromise`，清快取後其仍在飛、持有已刪快取句柄；`downloadModel(progressCallback)` → `ensurePipelineLoaded(progressCallback)` 見 `loadPromise` 存在**直接複用並丟棄新 callback** → 陳舊載入從已刪快取讀截斷位元組 → ORT session 建立失敗 → wasm 初始化失敗（`[non-Error number] 1025635888`）且 **Offscreen 生命週期不可恢復**，之後每次下載重複失敗。修復：①`cacheGeneration` 世代計數——`clearModelCache()` 遞增世代 + `disposePipeline()`（`Pipeline.dispose()` 釋放 ORT wasm session）+ 重置 `loadPromise`/`loadPromiseHasProgress`；②載入完成世代比對——`gen !== cacheGeneration` → `disposePipeline(p)` + 不落地 `translationPipeline` + 拋 `ModelCacheClearedError`（陳舊載入靜默 dispose + 診斷留痕，§5.6）；③`loadPromiseHasProgress` 進度防護——既有載入無進度回調而本次需進度時不複用，等待其終結（成功/失敗/作廢）後以帶進度新鮮載入承接；④`downloadModel` catch 區分 `ModelCacheClearedError` → 專屬診斷碼 `local-onnx-download-stale-load`。測試 +5 → **336**。先前：**補充修復十四：晚捕獲事件驅動重試**：真實環境「捕獲成功但字幕永久空白」根因——pot 重驅動鏈（無 pot 掛起 → 2s 排程 → 播放器帶 pot 重發 → 響應）超過 `waitForCaptureTimeoutMs=15s` 窗口；捕獲最終成功（51972 bytes）但 native 已 `no-caption-strategy` 永久降級、無重試路徑。修復（事件驅動、不提高固定等待窗口）：①`TimedTextBridge.onCapture(cb)` 訂閱 API——僅**新捕獲**觸發（`capturedAt` 去重過濾 1.5s 重播），返回 unsubscribe，stop/dispose/clearLatest 狀態一致（R4）；②`LateCaptureRetry` 純邏輯狀態機（application 層，可單測）——`arm(videoId)`/`disarm()`/`onCapture()` 三守衛（videoId 比對、`maxRetries=3`、`cooldownMs=5000`）+ `latencyMs` 延遲儀表；③content-script `pipeline-error(no-caption-strategy)` 置位 → 訂閱 bridge 捕獲 → `retryAfterLateCapture()` **輕量重跑 Orchestrator**（`orchestrator.start()` 內部先 stop 再重建鏈，不重掛 overlay/不重讀配置），native `tryReuseCapture` 命中晚捕獲；`segments-ready`/`stop()` 解除等待。測試 +16 → **331**。先前：**補充修復十三：local-onnx 翻譯首響應卡死修復 + 模型預加載機制**：真實環境捕獲成功（431 段）後字幕仍不出——日誌 `cues: 0` 持續 71s、錯誤全為 `Offscreen Document response timeout`（SW 30s）。根因：Offscreen 首次收到翻譯請求時模型未載入記憶體，lazy 載入 350MB 需 30-60s 而 SW `sendToOffscreen` 超時僅 30s → 首塊 request 被誤殺、provider 分 87 chunks（431/5）逐個 30s 超時 → 整體卡 71+ 秒才降級原文。修復：①`sendToOffscreen` 超時 30s→120s；②新增 `local-onnx:warmup` 消息 + `warmupModel()`（快取存在即載入並等待完成）；③`TranslationProvider` 介面新增可選 `warmup?()`；④`LocalONNXTranslationProvider` 實作 warmup（失敗落 `local-onnx-warmup-failed` 診斷）；⑤Orchestrator 啟動時 `void primary.warmup().catch(→ engine-degraded)` 非阻塞預熱；⑥Options「預加載模型」按鈕（手動 + 自動觸發式雙模式）；⑦模型同 offscreen 生命週期只載入一次。測試 +13 → **315**。先前：**補充修復十二：timedtext 空響應（pot 防護）主動重驅動**：真實環境「重新加載插件後完全無字幕」根因——播放器先發無 pot 請求 → YouTube 返回空 body（`length:0, text/html`）→ 攔截器 skip；播放器**稍後**才內部用 pot 重試，但 `ensureCaptionModuleLoaded` 選軌後**固定 3 秒復位** `setOption('captions','track',{})`（M1-48 抑制原生字幕所加）打斷了尚未完成的 pot 重試鏈 → 播放器永不再發第二次請求 → `waitForCapture(15s)` 超時 → 直接 fetch（無 pot）空 body → native 全鏈失敗。修復：①空響應視為「需要 pot」信號——`emitCapture` 空響應分支排程 `schedulePotRedrive()`（2s 間隔、上限 6 次），切換軌 off→on（先復位再重選，YouTube 對重複 setOption 可能 no-op）逼播放器重發帶 pot 的請求；②復位語義反轉——移除固定 3s 復位，改為「成功捕獲後 ~800ms 抑制原生字幕」（`scheduleSuppressNative`）+ 10 秒截止復位（`scheduleSuppressDeadline`，捕獲失敗時保留原生英文字幕而非全空）；③成功捕獲/video-changed 時 `resetPotRedrive()` 重置計數並取消排程；④調試全局 `__aiTransTimedtextEmptyResponses`/`__aiTransPotRedriveAttempts`。測試 +5（interceptor）。先前：**補充修復十一：local-onnx 輸出回顯原文 + 消息形狀不匹配**：主引擎 local-onnx、模型已下載，字幕仍雙語兩行相同英文、零翻譯錯誤日誌。根因：①消息 shape 不匹配——provider 發 `payload:{text}`、Offscreen 兩入口讀頂層 `msg.text` → 推理收到空文本；②純文本指令對 Qwen2.5-0.5B 遵循度極低 + `max_new_tokens:96` 太短 → 模型回顯原文當「譯文」（成功故無錯誤）。修復：消息統一 payload 優先 + port 通道建立時 direct 入口跳過（防雙重推理）；`buildPrompt` 改 ChatML + 行號標記 + 目標語言 few-shot（Qwen 全系列 ChatML 兼容，換大模型單點改動）；`parseNumberedOutput` 行號對齊 + 全回顯標記 `local-onnx-echo-output` 診斷；`max_new_tokens:256` + `repetition_penalty:1.1`；provider `CHUNK_SIZE=5` 分塊；空譯文回退原文；manifest WAR 加 `src/runtime/ort/*`（content-script 宿主頁面 fetch 擴充資源需 WAR，補充修復七只改 wasmPaths 漏 WAR）。先前：**補充修復九：本地 ONNX 推理失敗診斷可讀化 + 模型載入彈性化**：模型「已就緒」後播放仍雙語兩行英文——Offscreen 顯示模型載入成功（`model ready`）後推理拋 `Error: 1835858576`（ORT wasm 層**數字型**錯誤碼）。修復：`offscreen.ts` 抽共享 `loadPipeline()`（統一 env：`logLevel='info'` 輸出 `[ort]` 初始化/推理日誌、`wasmPaths` 本地化、**numThreads 交由 transformers.js 自決**——Offscreen 無 `crossOriginIsolated` 自動降 1）與 `ensurePipelineLoaded()`（並發安全）；`runInference` **lazy 載入**（Offscreen 重啟後從 Cache API 快取自動恢復，無需重新下載）；`checkModelStatus`/`hasModelInCache` 改用 **Cache API**（transformers.js v3 快取在 Cache API 而非 IndexedDB，且不依賴內存 `translationPipeline`）；`toReadableError` 診斷保留 code/stack（數字型錯誤碼可讀化）；生成參數簡化（貪婪解碼、`max_new_tokens` 128→96）；`LocalONNXTranslationProvider` 增 `isPrimary`（主引擎成功不誤發 engine-degraded）。先前：**補充修復八：local-onnx 可作為主翻譯引擎**：本地 ONNX 模型下載就緒後雙語仍兩行英文——根因：`TranslationConfig.type` 僅 `cloud-llm/local/mt`，local-onnx 只能當 fallback；MT 字典替換永不拋錯被判「成功」→ fallback 永不觸發。修復：`type` 增 `'local-onnx'`，Orchestrator primary 選擇、composition 註冊條件（`type==='local-onnx' || fallbackType==='local-onnx'`）、Options 下拉、popup 顯示同步。**譯文=原文的降級根源**：字典/佔位引擎「成功但未翻譯」不被管線識別，離線引擎須可直接作 primary；先前：**補充修復七：LocalWhisperASR WASM 本地化（content-script 宿主 CSP 阻擋 CDN）**：本地 ONNX 翻譯已就緒、字幕仍僅原文（雙語兩行英文）——YouTube 頁面報 `LocalWhisperASR warmup failed: no available backend found ... jsep.mjs`，CSP 違規：`Loading the script 'https://cdn.jsdelivr.net/...' violates ... "script-src 'self' 'wasm-unsafe-eval' ..."`。根因：先前只對 Offscreen（翻譯模型）做 wasm 本地化，content-script 的 LocalWhisperASR 未設 `wasmPaths`，仍走 jsdelivr CDN；content-script 運行於 **YouTube 頁面環境**，宿主 CSP 攔所有外部 script。修復：`LocalWhisperASR.warmup()` 動態導入 transformers.js 後設 `env.backends.onnx.wasm.wasmPaths = chrome.runtime.getURL('src/runtime/ort/')`（與 Offscreen 同源）。**教訓**：凡 transformers.js pipeline 執行環境（Offscreen 與 content-script）都必須設 `wasmPaths` 指向擴充內資源；content-script 的 CSP 屬宿主頁面，外部 CDN script 一律被攔；先前：**補充修復六：MV3 擴充頁面 CSP 需 `wasm-unsafe-eval`**：jsep 打包後仍失敗，Offscreen 報 `WebAssembly.instantiateStreaming() ... violates "script-src 'self'"` → `no available backend found`——MV3 擴充頁面默認 CSP 不含 wasm 編譯許可。修復：`manifest.json` 添加 `content_security_policy.extension_pages: "script-src 'self' 'wasm-unsafe-eval'; object-src 'self';"`；先前：**補充修復五：onnxruntime-web 需 jsep 變體**：wasm 本地化後仍失敗，Offscreen 報 `no available backend found ... Failed to fetch dynamically imported module: .../ort-wasm-simd-threaded.jsep.mjs`——onnxruntime-web v1.22 初始化 wasm backend 時 dynamic import **jsep 變體**（`ort-wasm-simd-threaded.jsep.{mjs,wasm}`），先前只打包非 jsep 致 404。`copy-static.mjs` 改為打包 jsep + 非 jsep 共 4 個文件（`src/runtime/ort/`）；先前：**補充修復四：ONNX WASM 本地化（CDN 不可達）**：模型可下載、進度 100% 後 Options 狀態卻顯示「下載失敗」。根因：transformers.js v3.8.1 默認 `wasmPaths = https://cdn.jsdelivr.net/npm/@huggingface/transformers@3.8.1/dist/`（onnx.js:211），模型文件（HF）下載完後創建 InferenceSession 需從 jsdelivr 拉取 `.wasm`——網絡不可達 → `pipeline()` 拋錯 → `ok:false`。修復：`copy-static.mjs` 拷貝 `node_modules/onnxruntime-web/dist/ort-wasm-simd-threaded.{wasm,mjs}` 到 `dist/src/runtime/ort/`，`offscreen.ts` 設置 `env.backends.onnx.wasm.wasmPaths = chrome.runtime.getURL('src/runtime/ort/')`（MV3 默認 CSP `'self'` 允許擴充頁面同源載入 wasm/module worker，無需改 manifest）；先前：**補充修復三：本地 ONNX 模型改用公開版（gated 401）**：原模型 `onnx-community/Qwen2.5-0.5B-Instruct-onnx-web` 為 gated（HF 401 需 Token），transformers.js v3.8.1 無法注入 Token，改用公開模型 `onnx-community/Qwen2.5-0.5B-Instruct` + pipeline `dtype: 'q4'`（INT4 `onnx/model_q4.onnx` ~350MB）；先前：**M2-24 本地 ONNX 翻譯兜底（F-14）**：§7.5 新增 `LocalONNXTranslationProvider` 適配器文檔——當雲端 LLM API 失敗時自動降級至本地 ONNX 小模型（`onnx-community/Qwen2.5-0.5B-Instruct`，約 350MB）進行翻譯。架構：content-script → Service Worker → Offscreen Document（`@huggingface/transformers` pipeline + IndexedDB 快取）。新增 `local-onnx:*` 消息協議（check-status/download/clear-cache/translate）。Options UI 新增「本地兜底模型」分區（狀態標籤 + 進度條 + 下載/清除按鈕）。`TranslationConfig.fallbackType` 新增 `'local-onnx'` 選項，降級鏈路：`local-onnx > mt > undefined`）；先前：2026-08-10（**M2-17/M2-18 治理回填**：§7.1 補 M2-17 CSP 違規修復（`new Function` → 直接 import）+ M2-18 ASR warmup 模塊解析（external 方案推翻 → esbuild 打包 transformers 進 IIFE）+ M2-18 字幕攔截器 DOM 解析（`getCaptionTracksFromPlayerResponse()` 首要來源兜底 `getOption` 空陣列）；§12 里程碑映射增列 M2-16/17/18）；先前：回填治理缺口 M1-48/M1-49/M1-50；§7.5 補 M1-48 LLM 直接 fetch 架構、§7.6 補 M1-49/M1-50、§12 增列 M1-48/49/50；§5.1 目錄樹補 `debug-log.ts`、§6.6 補調試日誌門控（F-12/M1-51）；§7.5 補 F-13/M1-52 分塊翻譯 + M1-53 兩階段超時；先前：M1-41/M1-42/M1-43/M1-45/M1-46/M1-47）））
...
export interface EngineConfig {
  ...
  asr: {
    ...
    modelTier?: 'tiny' | 'base' | 'small' | 'tiny-multi' | 'base-multi' | 'small-multi';
    endpoint?: string;
    apiKeyRef?: string;
  };
  targetLang: string;
...
> **Seek 響應與動態優先級翻譯（M1-57）**：`Orchestrator` 的 `observePlayback` 回調偵測 `currentTime` 突變 >10s（`SEEK_THRESHOLD_MS`），200ms debounce 後調用 `chain.onSeek(currentTime)`；`CaptionStrategyChain.onSeek()` 遍歷所有策略調用可選 `onSeek()`。`NativeCaptionStrategy` 實作：`onSeek()` 設置 `hasSeek` 標記 + `AbortController.abort()` 中斷當前翻譯；`translateWithPriority()` 循環每輪按 `getPrioritizedSegments(currentTime, windowStartOffsetMs(ctx.config.translation))` 排序——滑動窗口 `[currentTime+offset, currentTime+120s]` 內優先、窗口外按時間序填充；`translatedIds: Set<string>` 追蹤已翻譯 segments 避免重複；`mergeAccumulated()` 支持 provider 累計 emit 語義（更新已存在 + 添加新的）。`TranslationRequest.signal?: AbortSignal` 透傳至 `LocalONNXTranslationProvider.translateStream`（每 chunk 前檢查 `signal.aborted`，中止拋 `DOMException('AbortError')`）；`TranslationPipeline.translateStream` 偵測 `AbortError` 直接向上拋出（不觸發 fallback）。

> **滑動窗口起點依引擎區分（M1-59）**：M1-57 的滑動窗口起點統一 `currentTime+2s`，真實場景暴露兩問題——本地 ONNX 推理慢（每 chunk 5 段 + 每次 sendMessage 往返）需要更大提前量，`+2s` 在 seek 到遠位置後仍需先翻譯「稍早」的段、首塊字幕延遲增加；雲端/本地 LLM/MT 引擎 `+2s` 則讓 seek 後字幕視覺上「晚 2 秒」。**方案（依引擎區分）**：本地 ONNX（`config.translation.type === 'local-onnx'` 或 `fallbackType === 'local-onnx'`）窗口起點 `currentTime+5s`（`WINDOW_START_OFFSET_ONNX_MS=5000`，給慢推理留提前量、優先翻譯播放點前的段）；其餘引擎 `currentTime+0s`（`WINDOW_START_OFFSET_DEFAULT_MS=0`，窗口直接從當前播放點開始）。**引擎偵測**：`ctx.translation.engineId` 在 `TranslationPipeline` 包裝下恆為 `'pipeline'`，無法反映內部兜底引擎，故改用**配置偵測**（主/兜底任一為 `local-onnx` 即 +5s）。抽取純函數 `windowStartOffsetMs(translation: TranslationConfig): number`，`getPrioritizedSegments(currentTime, offsetMs)` 簽名由常量改為參數化偏移；`translateWithPriority` 以 `windowStartOffsetMs(ctx.config.translation)` 計算後傳入。
...
>
> **Whisper English-only 模型推理參數（M2-60）**：Whisper `.en` 變體（如 `Xenova/whisper-base.en`）是 English-only 模型，transformers.js v3 對其傳入 `language`/`task` 參數時拋出 `Cannot specify 'task' or 'language' for an English-only model`。`offscreen.ts` 新增 `asrPipelineModelId: string | null` 全局變量（`warmupAsrPipeline()` 載入後設置）；`runAsrInference()` 檢測 `asrPipelineModelId?.includes('.en')`，若是則構建 options 時**省略** `language`/`task`（只保留 `return_timestamps: true`）。當 `hintLang` 非英文（`!/^en/i.test(hintLang)`）且模型為 `.en` 時，`console.warn` 診斷警告（§5.6 不靜默）。多語言模型選項：`WHISPER_MODELS` 新增 `tiny-multi`/`base-multi`/`small-multi`（→ `Xenova/whisper-tiny`/`base`/`small`，非 `.en` 變體），支持任意語言 `hintLang`；`ASRConfig.modelTier` 類型擴展 6 值。**M2-71：默認改多語言**——`PROFILE_DEFAULTS` 三檔（streaming/balanced/quality）與 `DEFAULT_CONFIG.asr.modelTier` 皆改為 `-multi` 變體（tiny-multi/base-multi/small-multi），因多語言模型 `generation_config.json` 自帶 `is_multilingual: true` + `lang_to_id`/`task_to_id`，傳入 `language`/`task` 即可正確轉錄非英語音頻；English-only（`.en`）變體仍供用戶手動選擇（英文視頻更小更快）。

> **tabCapture.getMediaStreamId 必須在 Service Worker 中調用（M2-52 / M2-67）**：Chrome 116+ 將 `chrome.tabCapture.getMediaStreamId()` 返回的 streamId 限定在同一 render process——popup 取得的 streamId 只能在 popup 自身的 render process 中使用，無法跨進程傳遞給 offscreen document（屬不同 render process），導致 offscreen 調用 `getUserMedia(streamId)` 時拋 `AbortError: Error starting tab capture`。Service Worker 與 offscreen document 共享同一 extension context，SW 取得的 streamId 可在 offscreen 中正常使用。**正確架構**：Popup 不直接調用 `getMediaStreamId`，改為 `chrome.runtime.sendMessage({ topic: 'asr:get-stream-id' })` → SW 處理器以 callback 風格調用 `chrome.tabCapture.getMediaStreamId({}, cb)` → 回傳 `{ ok: true, streamId }` → Popup 再透過 `chrome.tabs.sendMessage` 交付至 content-script。SW 處理器同時須對 `chrome.runtime.lastError` 與例外做防禦（§5.6 不靜默）。**M2-67 release-stream 前置步驟**：Chrome `getMediaStreamId` 對同一 tab 已有 active capture stream 時直接拒絕（"Cannot capture a tab with an active stream."）。SW handler 在調用 `getMediaStreamId` 前先經 offscreen-onnx port 發 `asr:release-stream`（best-effort，port 不存在時跳過）；offscreen 收到後執行 `detachAudioProcessing()` + stop tracks + `mediaStream = null`，確保舊 stream 完全釋放。
...
> **Offscreen 單例衝突健壯化（M2-69）**：Chrome MV3 限制同一 extension 只能存在一個 offscreen document。`service-worker.ts` `ensureOffscreenDocument()` 原以 `getContexts({ OFFSCREEN_DOCUMENT, documentUrls: [OFFSCREEN_URL] })` 檢查是否已存在；真實 Chrome 中該 URL 精確匹配可能漏判（時序/URL 字串差異）→ 返回空 → 誤判「不存在」→ `createDocument()` → Chrome 拋「Only a single offscreen document may be created」。**修復**：①`getContexts` 改為不帶 `documentUrls` 過濾（僅按 contextType 匹配，更寬鬆可靠）；②`createDocument()` 被拒且錯誤含「single offscreen document」時重查 `getContexts`，確認已存在則視為成功（不拋錯）。另 `LocalWhisperASR.warmup()` 偵測到「single offscreen document」錯誤時先發 `offscreen:ensure-created`（讓 SW 重查/重建）再重試一次 warmup，覆蓋 SW `offscreenPort` 指向已關閉舊 document 的 port 路徑失敗情境。此修復解決「ASR warmup 失敗 → realtime-asr 策略整體降級 → 中英文字幕皆不出現」的根因（失敗發生在 warmup 階段、音頻流之前，與 VAD/累積窗口/模型檔位無關）。

> **VAD 能量閾值（M2-07 / M2-68）**：`src/infrastructure/vad.ts` 實裝 `EnergyVAD` 類——計算 `AudioChunk.pcm` 的 RMS 能量，低於閾值（`EngineConfig.asr.vadThreshold`，**M2-68：默認 0.005**）標記 `isSpeech = false`（靜音，跳過 ASR 節省算力）。靜音連續超過 2s 觸發分段邊界（切分 AudioChunk 送 ASR）。**M2-68 配置化**：Options UI 新增 VAD 閾值滑桿（0.1%~5%，step 0.1%，內部 ×0.01）；`RealtimeASRStrategy.baseVadThreshold` 同步改為 0.005。
...
> **本地 ONNX 翻譯（M2-24，F-14）**：`onnx-community/Qwen2.5-0.5B-Instruct`（INT4 ONNX，模型倉庫總計約 750MB）執行離線翻譯。可作**主翻譯引擎**（`TranslationConfig.type='local-onnx'`）或 **fallback 兜底**（`fallbackType='local-onnx'`）。
...
> **補充修復十三架構細節（M2-24，F-14，翻譯首響應卡死 + 模型預加載）**：
> - **SW 超時提升**：`sendToOffscreen` 的 response 超時 30s→120s（`service-worker.ts`）——首次翻譯請求可能觸發 350MB 模型載入（30-60s），30s 會把首塊 request 誤殺；後續推理（模型已載入）遠快於此，120s 僅為安全網。
> - **`local-onnx:warmup` 消息 + `warmupModel()`**（`offscreen.ts`）：新增 `OffscreenRequest` 類型 `{ type: 'local-onnx:warmup' }` 與響應 `{ type: 'local-onnx:warmup-complete'; ok; error? }`；direct onMessage 與 port 兩入口皆處理。`warmupModel()` **等待載入完成**才返回（與 `checkModelStatus` 的非阻塞後台預熱不同）——快取存在 → `ensurePipelineLoaded()`；已載入 → 直接 ok；無快取 → `ok:false 'Local ONNX model not downloaded'`；載入失敗 → `ok:false` + `local-onnx-warmup-failed` 診斷（§5.6）。
> - **`TranslationProvider` 介面新增可選 `warmup?()`**（`translation-provider.ts`）：本地引擎（local-onnx）實作；雲端 LLM 無需預熱可不實現——Orchestrator 用 `if (primary.warmup)` 守衛調用，保持端口最小。
> - **`LocalONNXTranslationProvider.warmup()`**：發送 `local-onnx:warmup`，`ok:false`/通信失敗皆拋錯並落 `local-onnx-warmup-failed` 診斷。
> - **Orchestrator 啟動預熱**：組裝 `TranslationPipeline` 後、策略鏈啟動前，`void primary.warmup().catch(err → engine-degraded(port: 'translation', reason: 'Translation warmup failed: …'))`——非阻塞（不延遲策略啟動），失敗發降級事件可見（§5.6）。
> - **Options「預加載模型」按鈕**（OMLX 風格手動預載入）：模型已下載（check-status 快取存在）時可用，點擊發送 `local-onnx:warmup`，成功顯示「已預加載（記憶體）」；未下載時禁用；下載/清除/預加載三按鈕互斥啟用狀態。**自動觸發式加載**由 Orchestrator warmup 承擔，雙通道消除首次推理抖動。
> - **只載入一次**：`translationPipeline` 為 offscreen 模組級變量，同一 offscreen 生命週期內複用（跨視頻不需重載）；Chrome 銷毀 offscreen 後 `runInference` lazy 恢復。

> **補充修復十五架構細節（M2-24，F-14，清快取後重新下載永遠失敗）**：
> - **`cacheGeneration` 世代計數**（模組級）：每次 `clearModelCache()` 遞增——使「載入期間快取被清除」的 in-flight 載入結果作廢。`clearModelCache()` 現同時：遞增世代 + `disposePipeline()` 舊 pipeline（釋放 ORT wasm session）+ 重置 `translationPipeline`/`loadPromise`/`loadPromiseHasProgress`。
> - **載入完成世代比對（`ensurePipelineLoaded`）**：`loadPipeline(progressCallback).then` 內在落地前檢查 `gen !== cacheGeneration` → `await disposePipeline(p)`（釋放記憶體）+ **不落地** `translationPipeline` + 拋 `ModelCacheClearedError`（`Error` 子類，message `local-onnx: model cache cleared during load`）。陳舊載入**靜默 dispose + 診斷留痕**（§5.6：不靜默吞、不誤報成功——由 `downloadModel` catch 落 `local-onnx-download-stale-load`；預熱路徑由 `checkModelStatus` 的 catch 落 `local-onnx-pipeline-warmup-failed`）。
> - **`loadPromiseHasProgress` 進度防護**：共享載入記錄「本次是否帶進度回調」。`ensurePipelineLoaded(progressCallback)` 在「既有載入無進度回調且本次需進度」時**不複用**——`await loadPromise`（成功/失敗皆可）後再判斷：`translationPipeline` 已落地 → 直接返回；世代已變 → 拋 `ModelCacheClearedError`；否則以**帶進度的新鮮載入**承接。杜絕「下載點下去進度恆 0、實際複用預熱載入」。
> - **`disposePipeline(pipeline)`**：`typeof pipeline.dispose === 'function'` 時 `await pipeline.dispose()`——transformers.js `Pipeline.dispose()` 連帶釋放底層 ONNX session 的 wasm 記憶體；反覆清/載不釋放會使 wasm 堆膨脹，最終觸發 OOM 型 wasm trap。
> - **`downloadModel` 診斷碼區分**：catch 中 `err instanceof ModelCacheClearedError` → code `local-onnx-download-stale-load`（recoverable，提示「快取被清除、需重新下載」）；否則維持 `local-onnx-download-failed`。讓 popup「最近失敗」能區分「被清快取打斷」與「下載真的失敗」。
> - **`_testExports` 增補**：新增 `downloadModel`/`ensurePipelineLoaded` 匯出供集成測試直接驅動載入流程；`resetLocalOnnxModuleForTest` 同步重置 `cacheGeneration`/`loadPromiseHasProgress` 新狀態，避免用例間污染。

> **補充修復十六架構細節（M2-24，F-14，local-onnx 流式漸進 + echo 診斷儀器）**：
> - **`translateStream` 真流式化**：`LocalONNXTranslationProvider` 原 `translateStream` 為 `await this.translate(req)`（全量 87 chunk 串行推理後才 emit 一次）——`NativeCaptionStrategy` 僅首次 emit 發 `segments-ready`，長視頻數分鐘無渲染。修復：抽取私有 `translateChunk(chunk, targetLang): Promise<{ segments; echoed }>`（合併 sourceText → sendMessage → 按行拆分對齊，echo 標記來自響應 `echoed` 字段）；`translateStream` 逐 chunk `emit` **累計全量** `{ engineId, degraded: !isPrimary, segments: [...accumulated] }`（與 LLM M1-52 同語義：首塊數秒 `segments-ready`、後續 `segments-updated`）；每塊 `diagLog('local-onnx', 'chunk i/N done ...')`。`translate()` 行為不變（共用 `translateChunk`）。中段 chunk 失敗 → 拋錯 → pipeline R5 兜底（fallback + `engine-degraded`），與 LLM 語義一致。
> - **`parseNumberedOutput` 返回 `parsedCount`**：`{ translatedLines, echoed, parsedCount }`——`parsedCount` 為實際解析到「行號→譯文」的行數。讓 echo 診斷能分辨「模型輸出了譯文但未帶行號/格式不符（`parsedCount` 低）」vs「真回顯」（`parsedCount` 高但譯文==原文）。解析規則本身不變（容錯回退待診斷確認後再定）。
> - **echo 診斷 cause 內嵌 raw 輸出**：`runInference` 的 `local-onnx-echo-output` 診斷 cause message 改為 `local ONNX echoed input (parsed ${parsedCount}/${sourceLines.length} lines, took ${elapsedMs}ms); raw output: ${JSON.stringify(generatedText.slice(0,200))}`——popup「最近失敗」直接看到模型實際輸出與解析統計。
> - **響應新增 `echoed?: boolean`**：`OffscreenResponse['local-onnx:translate-result']` 與 `LocalOnnxTranslateResponse` 皆新增 `echoed`——Offscreen 判定結果透傳給 provider，供流式路徑統計低質量 chunk。
> - **聚合診斷 `local-onnx-echo-chunks`**：`translateStream`/`translate` 結束時若 `echoedChunks > 0` → `recordDiagnostic` code `local-onnx-echo-chunks`，cause `local ONNX model echoed input in X/Y chunks (low quality output)`。純診斷、不做任何降級（用戶指令：僅診斷）。
> - **`local-onnx` 第九調試分類**：`DebugLogCategory` 新增 `'local-onnx'`，`DEBUG_LOG_OFF`/Options checkbox/`DEBUG_CATEGORY_IDS` 同步——provider 逐 chunk 麵包屑受門控（默認關），§5.6 錯誤/降級不受開關影響。

> **M2-26 架構細節（WebGPU 翻譯後端 + popup init watchdog + manifest commands 移除）**：
> - **WebGPU-first 翻譯後端**（`offscreen.ts loadPipeline`）：本地 ONNX **翻譯** pipeline 載入改為 **webgpu-first**——先試 `device: 'webgpu'`（權重駐留 GPU VRAM、JS heap 從 ~500MB 降到 ~50-100MB，解決「模型載入把共享 extension 渲染進程堆撐爆 → popup 創建失敗」）；WebGPU 不可用/初始化失敗時置模組級 `webgpuFailed = true`、落診斷碼 `local-onnx-webgpu-fallback`（port 'translation'、degraded/recoverable），隨後 `preferWebGpu()` 返回 false → **直走 wasm、不重試**（避免每次載入都先撞一次 WebGPU 失敗延遲）。ASR whisper pipeline **維持 `device: 'wasm'` 不變**（僅翻譯後端換 webgpu；whisper tiny/base 模型小、堆壓力可接受）。新增 `logJsHeapBreadcrumb(label)`（`performance.memory?.usedJSHeapSize` 可選鏈守衛、單位 MB）於 `loadPipeline` 成功 + `shutdownForIdle` 前各打一次 Offscreen console warn，讓堆佔用可被觀察。`resetLocalOnnxModuleForTest` 重置 `webgpuFailed = false`。
> - **popup init watchdog**（`popup.ts`）：`startInitWatchdog()` 於 popup 打開即設 **3s（`popup-init-slow`，degraded）+ 10s（`popup-init-timeout`，error）**兩支 timer（port 'platform'）；成功 init 調 `stopInitWatchdog()` 清雙 timer；**timeout 獨立觸發**（不受 slow 旗標守衛阻擋——10s 是比 3s 更嚴重的事件，應獨立記錄並覆蓋 slow 的最終值）。讓「popup 打不開/卡住」這類基礎設施故障有儀器記錄，不再只靠用戶主觀猜測（§5.6）。
> - **manifest `commands` 移除**：先前未合併的「快捷鍵開 Options」功能（自訂 command `open-options`）在 E2E 的 headless Chromium 上令**整包 SW 註冊失敗**（`_execute_action` 內建命令可正常註冊、自訂 command 任何鍵值皆不行）→ 擴充根本加載失敗（E2E 9/16 掛）。決策：移除 manifest `commands` 塊；Options 改經 popup「開啟設定」按鈕（`openOptionsPage()`，`chrome.tabs.query/update/create`）到達，無用戶能力損失。**同步移除 SW 死監聽**（補充修復）：`chrome.commands.onCommand` 監聽整段刪除——manifest 無 `commands` 鍵時真實 Chrome **裁剪該命名空間**（`chrome.commands === undefined`），頂層未守衛引用令 SW 求值拋 `TypeError` → 註冊失敗（status 15）→ 整包擴充失效；jsdom mock 與 E2E Chromium 都注入該命名空間故無法暴露，僅用戶端現形。測試環境（setup-dom）移除 `commands` mock 貼近真實 + 回歸測試斷言 SW 在 `chrome.commands` 未定義時可正常求值。
> - **SW 生命週期麵包屑**（`service-worker.ts`）：`ensureOffscreenDocument` 成功建檔後 `console.warn('[AI_Trans:sw] offscreen created')`；檔尾以 `if (chrome.runtime.onX)` 守衛掛 `onStartup`/`onInstalled`/`onSuspend` warn——讓 SW 空閒回收（~30s）/重啟在 console 可見，定位「SW 被殺後消息無回應」類問題。

> **M2-27 架構細節（Options 模型狀態即時顯示——ONNX 五態 + ASR 下載中 + 背景預熱廣播）**：
> - **offscreen 狀態字段擴充**（`offscreen.ts`）：`local-onnx:status` 響應類型新增 `loaded?: boolean`（`translationPipeline !== null`，已預加載到記憶體）/ `loading?: boolean`（`loadPromise !== null && !loaded`，預加載進行中）/ `downloading?: boolean`（恆 `false`——local-onnx 下載進度走 `downloadModel` 專屬進度廣播，不用狀態輪詢）；`asr-whisper:status` 響應新增 `downloading?: boolean`（`asrDownloadInProgress` 模組級旗標，`downloadAsrModel` 入口 set、出口 finally clear、`resetLocalOnnxModuleForTest` 同步重置）。`checkModelStatus()` 據 `translationPipeline`/`loadPromise` 推導 loaded/loading；`checkAsrModelStatus()` 回報 `downloading: asrDownloadInProgress`。`_testExports` 新增 `checkAsrModelStatus`/`downloadAsrModel`/`hasAsrModelInCache`。
> - **背景預熱完成廣播**：`checkModelStatus()` 觸發的後台 `warmupModel()`（快取存在即載入）完成後 `broadcastToAll({ type: 'local-onnx:status', loaded: true, ... })`——所有已開啟的 Options 頁即時收到最新狀態，無需重新 `check-status`。
> - **Options UI 五態**（`options.ts`）：新增 `applyModelStatus` 統一渲染邏輯——ONNX 依序判定「下載中（`downloading`）→ 已預加載（記憶體）（`loaded`）→ 預加載中（`loading`）→ 已就緒（`downloaded`）→ 未下載（兜底）」，同步驅動「預加載模型」按鈕 disabled 連動；ASR 判定「下載中（`downloading`）→ 已就緒（`downloaded`）→ 未下載」。`checkModelStatus` 改走 `applyModelStatus`。`initLocalOnnxModelUI` 註冊**一次性** `chrome.runtime.onMessage.addListener`（回調常駐、僅註冊一次避免隨開頁線性累積）收 `local-onnx:status` / `asr-whisper:status` 廣播即時刷新。
> - **測試基建**（`setup-dom.ts`）：runtime mock 的 `onMessage.addListener/removeListener` 由 `vi.fn()` 改為真正存入 `Set`——因 **Vitest 將 setupFile 與測試檔視為不同模組實例**，module-level 監聽器 Set 在兩實例間不共享，改為掛在 `chromeMock` 對象的 `_listeners` 上並由 `dispatchRuntimeMessage` 讀 `globalThis.chrome.runtime.onMessage._listeners`；新增 `dispatchRuntimeMessage(msg)` / `getOnMessageListenerCount()` 測試助手，`resetChromeMock` 清空該 Set。

> **M2-38 架構細節（Small model (MarianMT) 雙檔位支持 + 退化輸出檢測）**：
> - **雙檔位模型配置**（`config.ts`）：`LOCAL_TRANSLATION_MODELS` 常量定義兩檔位——`small: 'Xenova/opus-mt-en-zh'`（MarianMT，記憶體約 150MB，僅支援英→中）與 `large: 'onnx-community/Qwen2.5-0.5B-Instruct'`（通用 LLM，記憶體約 750MB）。`LocalModelTier = 'small' | 'large'` 類型，`TranslationConfig.localModelTier?: LocalModelTier` 配置項。
> - **檔位切換機制**（`offscreen.ts`）：`currentModelTier` 模組級變量追蹤當前載入檔位；`setModelTier(tier)` 切換檔位時先 `disposePipeline()` 釋放舊模型再重置 `translationPipeline = null`；`isSmallModel()` 輔助函數判斷當前是否 Small model；`smallModelBroken` 旗標標記 WASM 不相容（首次 WASM 錯誤重試失敗後置 true，後續請求直接返回錯誤）。`runInference` 接收 `requestedTier` 參數，若與 `currentModelTier` 不同則先調用 `setModelTier(requestedTier)` 切換。
> - **推理路徑分化**（`offscreen.ts runInference`）：Small model 使用 `pipeline('translation', modelName, { dtype: 'q8', device })` 載入，推理時直接輸入文本返回 `[{ translation_text: string }]`；Large model 使用 `pipeline('text-generation', modelName, { dtype: 'q4', device })` 載入，推理時構造 ChatML Prompt 返回 `[{ generated_text: string }]`。Small model 生成參數：`max_new_tokens: 256`、`repetition_penalty: 1.2`、`no_repeat_ngram_size: 3`（抑制重複循環）；Large model 生成參數：`max_new_tokens: 256`、`do_sample: false`、`repetition_penalty: 1.1`、`return_full_text: false`。
> - **退化輸出檢測**（`offscreen.ts`）：`calcUniqueNgramRatio(text, n)` 純函數計算文本中唯一 n-gram 佔總 n-gram 的比例（正常翻譯 80-100%，重複循環 <10%）。Small model 推理完成後、簡繁轉換前，對 `translation_text` 計算 3-gram 唯一率；當文本長度 >100 字符且唯一率 <0.2 時判定為退化輸出（重複循環），返回 `{ ok: false, degenerate: true }`。
> - **Provider 層處理**（`local-onnx-translation.ts`）：`CHUNK_SIZE_SMALL = 3`（Small model 逐句翻譯，速度 ~0.25 seg/s）、`CHUNK_SIZE_LARGE = 5`（Large model 合併多行）。`translateChunk` 收到 `degenerate: true` 響應後：①回退原文（`translatedText: seg.sourceText`）；②`recordDiagnostic` 落 `small-model-degenerate` 診斷碼；③`consecutiveDegenerateCount` 追蹤連續退化次數；④輸出提示建議用戶切換 Large model（不自動切換——用戶因硬件限制選擇 Small model 時保持選擇權）。`smallModelStats` 模組級統計對象追蹤 merged/perfect/fallback chunks 數量，每 10 個 chunks 輸出摘要日誌。
> - **消息協議擴展**：`LocalOnnxTranslateRequest.payload` 新增 `modelTier?: 'small' | 'large'` 字段，Offscreen 據此選擇正確推理路徑；`OffscreenResponse['local-onnx:translate-result']` 與 `LocalOnnxTranslateResponse` 皆新增 `degenerate?: boolean` 字段——退化標記從 Offscreen 透傳給 Provider。
> - **代碼落點**：`src/domain/models/config.ts`（`LOCAL_TRANSLATION_MODELS` + `LocalModelTier`）、`src/runtime/offscreen.ts`（`calcUniqueNgramRatio` + `setModelTier` + `isSmallModel` + `smallModelBroken` + `runInference` 雙檔位分支 + 退化檢測）、`src/adapters/translation/local-onnx-translation.ts`（`CHUNK_SIZE_SMALL/LARGE` + `translateChunk` 退化處理 + `consecutiveDegenerateCount` + `smallModelStats`）。

> **M2-39 架構細節（三檔位模型支持 + 檔位切換清理舊快取 + 默認檔位改為 Large）**：
> - **三檔位模型配置**（`config.ts`）：`LOCAL_TRANSLATION_MODELS` 常量擴展為三檔位——`small: 'Xenova/opus-mt-en-zh'`（MarianMT，~150MB，僅支援英→中）、`medium: 'onnx-community/SmolLM2-360M-Instruct'`（SmolLM2，~200MB，平衡質量與性能）、`large: 'onnx-community/Qwen2.5-0.5B-Instruct'`（Qwen2.5，~750MB，高質量）。`LocalModelTier = 'small' | 'medium' | 'large'` 類型自動擴展。
> - **默認檔位改為 Large**：`composition.ts`、`options.ts`、`offscreen.ts` 中所有 `?? 'small'` 改為 `?? 'large'`；`resetLocalOnnxModuleForTest` 重置為 `'large'`。Options UI `<select>` 默認 `selected` 設為 `large`，hint 文字更新為「推薦使用大型模型獲得最佳翻譯質量」。
> - **檔位切換清理舊快取**（`offscreen.ts`）：`setModelTier()` 改為 `async`，切換時除了 `disposePipeline()` 釋放內存外，新增 `clearCacheForModel(oldModelName)` 清除舊檔位模型的 Cache API 快取條目（按 `modelName` 精確匹配）。`hasModelInCache()` 修正為精確匹配 `currentModelName`（移除 `.onnx` 泛匹配，避免多檔位共存時誤判）。消息處理器 `local-onnx:set-model-tier` 改為 `await setModelTier()` 後查詢 `hasModelInCache()` 返回真實狀態（不再固定 `downloaded: false`）。
> - **Medium tier 推理路徑**（`offscreen.ts runInference`）：Medium model 使用 `text-generation` pipeline（與 Large 相同），但構造 SmolLM2 chat template prompt（`buildPromptSmolLM2`）而非 ChatML。SmolLM2 格式：`<|system|>...<|end|><|user|>...<|end|><|assistant|>`。`runInference` 根據 `currentModelTier` 選擇 prompt 構建函數：`medium` → `buildPromptSmolLM2`，`large` → `buildPrompt`（ChatML）。
> - **Provider 層 chunk size 擴展**（`local-onnx-translation.ts`）：新增 `CHUNK_SIZE_MEDIUM = 4`（Medium model 合併 4 行，介於 Small 3 行與 Large 5 行之間）。`chunkSize` getter 改為 `switch` 語句處理三檔位。`LocalOnnxTranslationConfig.modelTier` 類型改為 `LocalModelTier`（含 `'medium'`）。
> - **Options UI 更新**（`options.html` + `options.ts`）：`<select id="local-model-tier">` 新增 `<option value="medium">`；`MODEL_SIZES` 映射新增 `medium: '約 200 MB'`；Small model 選項文字加 `⚠ 質量不穩定` 警告。
> - **代碼落點**：`src/domain/models/config.ts`（`LOCAL_TRANSLATION_MODELS.medium`）、`src/runtime/offscreen.ts`（`clearCacheForModel` + `buildPromptSmolLM2` + `setModelTier` async + `hasModelInCache` 精確匹配 + `runInference` 三檔位分支）、`src/adapters/translation/local-onnx-translation.ts`（`CHUNK_SIZE_MEDIUM` + `chunkSize` switch）、`src/runtime/options/options.html`（medium 選項 + Small 警告）、`src/runtime/options/options.ts`（`MODEL_SIZES.medium` + 默認 `'large'`）。
...
> 
> **本地 LLM 服務 Origin 限制（M2-35）**：Chrome 擴展的 SW fetch 會自動攜帶 `Origin: chrome-extension://<id>` 頭部，無法通過 fetch API 剝離（Origin 是禁止設置的頭部）。部分本地 LLM 服務（如 Ollama 0.32+）的默認 Origin 白名單不包含 `chrome-extension://`，會返回 403 空響應體。解決方案：用戶需配置本地 LLM 服務的 Origin 白名單（如 Ollama 設置環境變量 `OLLAMA_ORIGINS=chrome-extension://*` 或 `OLLAMA_ORIGINS=.`）。連接測試檢測到 403 空響應體時顯示通用提示引導用戶配置。
> 
