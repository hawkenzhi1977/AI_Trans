...
> - **生成參數簡化**：`{ max_new_tokens: 96, do_sample: false }` 貪婪解碼（去除 `temperature`），降低 0.5B 模型在瀏覽器 wasm 的記憶體峰值與失敗概率。
