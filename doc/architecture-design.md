...
>
> **Whisper English-only 模型推理參數（M2-60）**：Whisper `.en` 變體（如 `Xenova/whisper-base.en`）是 English-only 模型，transformers.js v3 對其傳入 `language`/`task` 參數時拋出 `Cannot specify 'task' or 'language' for an English-only model`。`offscreen.ts` 新增 `asrPipelineModelId: string | null` 全局變量（`warmupAsrPipeline()` 載入後設置）；`runAsrInference()` 檢測 `asrPipelineModelId?.includes('.en')`，若是則構建 options 時**省略** `language`/`task`（只保留 `return_timestamps: true`）。當 `hintLang` 非英文（`!/^en/i.test(hintLang)`）且模型為 `.en` 時，`console.warn` 診斷警告（§5.6 不靜默）。多語言模型選項：`WHISPER_MODELS` 新增 `tiny-multi`/`base-multi`/`small-multi`（→ `Xenova/whisper-tiny`/`base`/`small`，非 `.en` 變體），支持任意語言 `hintLang`；`ASRConfig.modelTier` 類型擴展 6 值；`PROFILE_DEFAULTS` 保持 `.en`（多語言僅供用戶手動選擇）。
