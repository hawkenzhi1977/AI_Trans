...
| M1-27 | **真實 YouTube 頁面驗證**（手動冒煙 + 自動字幕接口容錯） | P1 | 4 | 🟡 進行中 | E2E（Mock 已驗證閉環）+ 待手動真實冒煙。診斷鏈已能區分「字幕軌抓不到」（三態）與「翻譯失敗」。**真實根因已定位**（診斷鏈逐步鎖定）：popup 先顯示 `lookahead-asr: not implemented (M3) | realtime-asr: not implemented (M2)`（native 原因被鏈吞，已修），再顯示 `native: run failed — timedtext XML: missing transcript root`——**真實 YouTube baseUrl 默認返回 srv3 XML（`<timedtext><body><p><s>`）或非字幕 HTML**，舊解析器只認 `<transcript><text>`，誤判「無字幕根」。已修（M1-40）：強制 `fmt=json3` + srv3 兜底解析 + HTML 錯誤頁識別。待用戶重新加載驗證 |
...
- **M1 尾項（P1，非阻塞）**：M1-27 真實 YouTube 頁面手動冒煙驗證。診斷鏈（M1-38/39）逐步逼近真實根因——popup 依次暴露 `no-caption-strategy` → `native: run failed — timedtext XML: missing transcript root`，最終定位為 **timedtext 真實返回 srv3 XML/HTML 而非傳統 transcript XML**，已修（M1-40：fmt=json3 + srv3 兜底 + HTML 錯誤頁識別）。下一步：用戶重新加載擴充、刷新 YouTube 頁，確認字幕是否正常顯示（若仍失敗，popup「最近失敗」會給出新的具體原因）。
- **下一步優先（M2）**：
