...
> 最後更新：2026-08-06（M1-44：LLM 翻譯 body 讀取失敗與非 JSON 區分診斷）
...
- **M1 尾項（P1，非阻塞）**：M1-27 真實 YouTube 頁面手動冒煙驗證。診斷鏈（M1-38/39）逐步逼近真實根因——popup 依次暴露 `no-caption-strategy` → `native: run failed — timedtext XML: missing transcript root`（M1-40 修：fmt=json3 + srv3 兜底 + HTML 錯誤頁識別）→ `timedtext XML: parse error (not valid XML) — root <html>, (empty body)`（**最終根因：YouTube pot token 防護**——無 pot 請求一律空 body，M1-42 修：MAIN world 攔截播放器 XHR 複用響應；M1-43 修補時序斷鏈：注入提前、等待捕獲窗口、雙 hook、stop 保留緩存，E2E 捕獲鏈路計數=1 全綠）。下一步：**用已登錄的 Chrome 手動加載 dist 擴充 + 打開真實 YouTube 視頻**，確認字幕正常顯示（這是攔截器首次在真實登錄環境成功抓字幕的關鍵驗證，尚未取得證據）。
- **下一步優先（M2）**：
