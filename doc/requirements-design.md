...
> 版本：v0.1（草案）
> 狀態：需求與可行性方案討論結論
> 最後更新：2026-08-20（**新增 F-15（M1-59/M2-27）**：①滑動窗口起點依引擎區分——本地 ONNX（主/兜底）`currentTime+5s`、其餘引擎 `currentTime+0s`，配置偵測引擎（`engineId` 在 pipeline 包裝下恆為 `'pipeline'` 不可用），純函數 `windowStartOffsetMs` + `getPrioritizedSegments(ct, offsetMs)` 參數化；②Options 模型狀態即時顯示——offscreen 狀態字段擴充（ONNX `loaded/loading/downloading?`、ASR `downloading?`）+ 背景預熱完成 `broadcastToAll` 廣播 + Options 五態渲染與 onMessage 監聽即時刷新。）
> 前次更新：2026-08-19（**補充修復十六：local-onnx 流式漸進 + echo 診斷儀器（F-14 增強——真實環境「字幕完全空白」根因修復 + 可診斷性）**：真實環境「翻譯啟動但字幕完全空白」根因——`LocalONNXTranslationProvider.translateStream()` 非真流式（全量 87 chunk 串行推理後才 emit 一次），`NativeCaptionStrategy` 僅首次 emit 發 `segments-ready` → 長視頻數分鐘無渲染；且 echo 診斷無 raw 輸出，無法分辨「真回顯」vs「解析誤判」（`parseNumberedOutput` 正則要求「數字+點+空格」，`N.譯文` 無空格或無行號輸出全回落原文誤判 echo）。新增需求：①**真流式**——`translateStream` 逐 chunk emit 累計全量（首塊數秒 `segments-ready`、後續 `segments-updated`，與 LLM 同語義），任何 provider 的流式都必須真流式（策略只在首次 emit 發 ready）；②**診斷儀器（純診斷、零降級）**——`parseNumberedOutput` 返回 `parsedCount`、echo 診斷 cause 內嵌 raw `generated_text` 前 200 字符 + `parsed x/N` + 推理耗時、響應新增 `echoed?: boolean`、多 chunk 回顯聚合 `local-onnx-echo-chunks` 診斷、新增 `local-onnx` 第九調試分類；③解析容錯（無行號/無空格回退）**延後**——待 raw 輸出診斷確認 echo 真偽後再定。先前：**補充修復十五：清快取後重新下載模型永遠失敗（F-14 增強——下載/快取狀態一致性）**：真實環境「清除快取 → 重新下載」必失敗——`clearModelCache()` 未重置共享載入狀態，`downloadModel` 複用 check-status 預熱建立且無進度回調的 in-flight 載入並丟棄新 callback → 從已刪快取讀截斷位元組 → ORT wasm 初始化失敗（`[non-Error number] 1025635888`）且 Offscreen 生命週期不可恢復。新增需求：①清快取 = 使在飛載入作廢——`cacheGeneration` 世代計數，載入完成世代比對不符則 dispose + 不落地 + 拋 `ModelCacheClearedError`；②進度防護——無進度回調的在飛載入不得被帶進度的下載複用（等待終結後新鮮承接）；③`clearModelCache` 同時 dispose 舊 pipeline 釋放 wasm 記憶體；④下載被清快取打斷落專屬診斷 `local-onnx-download-stale-load`，popup/Options 可區分「重試即可」與「真實失敗」。先前：**補充修復十四：晚捕獲事件驅動重試（F-04/F-11 增強——原生字幕捕獲可靠性）**：真實環境「捕獲成功但字幕永久空白」根因——pot 重驅動鏈超過 15s `waitForCapture` 窗口，捕獲最終成功（51972 bytes）但 native 已 `no-caption-strategy` 永久降級、無重試路徑。新增需求：捕獲到達作為**事件信號**——`TimedTextBridge.onCapture(cb)` 訂閱（`capturedAt` 去重）、`LateCaptureRetry` 狀態機（videoId/上限 3 次/冷卻 5s 守衛）、content-script `no-caption-strategy` 置位後晚捕獲自動輕量重跑 Orchestrator（不重掛 overlay/不重讀配置），`segments-ready`/`stop()` 解除等待；失敗落 `native-capture-late-retry` 診斷。先前：**補充修復十三：local-onnx 翻譯首響應卡死修復 + 模型預加載機制（F-14 增強）**：真實環境捕獲成功（431 段）後字幕仍不出——日誌 `cues: 0` 持續 71s、錯誤全為 `Offscreen Document response timeout`（30s）。根因：Offscreen 首次收到翻譯請求時模型未載入記憶體，lazy 載入 350MB 需 30-60s 而 SW `sendToOffscreen` 超時僅 30s → 首塊 request 被誤殺、87 chunks 逐個超時。修：lazy 載入 350MB 需 30-60s 而 SW `sendToOffscreen` 超時僅 30s → 首塊 request 被誤殺、87 chunks 逐個超時。修復：①SW 超時 30s→120s；②新增 `local-onnx:warmup` 消息 + `warmupModel()`（快取存在即載入並等待完成）；③`TranslationProvider` 介面新增可選 `warmup?()`；④`LocalONNXTranslationProvider` 實作 warmup；⑤Orchestrator 啟動時非阻塞預熱 primary；⑥Options「預加載模型」按鈕（OMLX 風格手動預載入 + 自動觸發式雙模式）；⑦模型同 offscreen 生命週期只載入一次。先前：2026-08-18（**補充修復八：local-onnx 可作為主翻譯引擎（F-14 增強）**：本地 ONNX 模型下載就緒後雙語仍顯示兩行英文——根因：`TranslationConfig.type` 僅允許 `cloud-llm/local/mt`，local-onnx 只能當 fallback；MT 字典替換（默認空字典、永不拋錯）被判「成功」→ fallback 永不觸發、譯文=原文。修復：`TranslationConfig.type` 增加 `'local-onnx'`，Options「引擎類型」新增「本地 ONNX 模型（離線）」，Orchestrator primary 選擇、composition 註冊、popup 顯示同步支持。先前：2026-08-18（**補充修復七：LocalWhisperASR WASM 本地化（content-script 宿主 CSP 阻擋 CDN）**：本地 ONNX 翻譯就緒後字幕仍僅原文（雙語兩行英文）——YouTube 頁面報 `LocalWhisperASR warmup failed: no available backend found ... jsep.mjs` CSP 違規。根因：先前只對 Offscreen（翻譯模型）做 wasm 本地化，content-script 的 LocalWhisperASR 未設 `wasmPaths` 仍走 jsdelivr CDN，而 content-script 運行於 YouTube 頁面環境、宿主 CSP 攔外部 script。修復：`LocalWhisperASR.warmup()` 動態導入後設 `env.backends.onnx.wasm.wasmPaths = chrome.runtime.getURL('src/runtime/ort/')`。先前：2026-08-18（**補充修復六：MV3 擴充頁面 CSP 需 `wasm-unsafe-eval`**：jsep 打包後仍「下載失敗」，Offscreen 報 `WebAssembly.instantiateStreaming() ... violates "script-src 'self'"`——MV3 擴充頁面默認 CSP 不含 wasm 編譯許可。`manifest.json` 添加 `content_security_policy.extension_pages: "script-src 'self' 'wasm-unsafe-eval'; object-src 'self';"`。先前：2026-08-18（**補充修復五：onnxruntime-web 需 jsep 變體**：wasm 本地化後仍「下載失敗」，Offscreen 報 `no available backend found ... Failed to fetch dynamically imported module: .../ort-wasm-simd-threaded.jsep.mjs`——onnxruntime-web v1.22 初始化 wasm backend 需 **jsep 變體**（`ort-wasm-simd-threaded.jsep.{mjs,wasm}`），`copy-static.mjs` 改為打包 jsep + 非 jsep 共 4 個文件。先前：2026-08-18（**補充修復四：ONNX WASM 本地化（CDN 不可達）**：模型可下載、進度 100% 後 Options 狀態卻顯示「下載失敗」。根因：transformers.js v3.8.1 默認從 `cdn.jsdelivr.net` 載入 onnxruntime wasm（模型文件下載完後創建 InferenceSession 需拉取 `.wasm`），網絡不可達 → `pipeline()` 拋錯。修復：`copy-static.mjs` 拷貝 `ort-wasm-simd-threaded.{wasm,mjs}` 到擴充 `src/runtime/ort/`，`offscreen.ts` 設置 `wasmPaths = chrome.runtime.getURL('src/runtime/ort/')`。先前：2026-08-18（**補充修復三：本地 ONNX 模型改用公開版（gated 401）**：實測下載報 `Unauthorized access to file`（HF 401）——原模型 `onnx-community/Qwen2.5-0.5B-Instruct-onnx-web` 為 gated（需登入 + Token），而 transformers.js v3.8.1 無 Token 注入機制，改用公開模型 `onnx-community/Qwen2.5-0.5B-Instruct` + `dtype: 'q4'`（INT4 `onnx/model_q4.onnx` ~350MB）。先前：2026-08-18（**新增 F-14 本地 ONNX 翻譯兜底（M2-24）**：當雲端 LLM API 失敗時自動降級至本地 ONNX 小模型（`onnx-community/Qwen2.5-0.5B-Instruct`，約 350MB）進行翻譯，實現完全離線的翻譯兜底。新增 `LocalONNXTranslationProvider` 適配器、Offscreen Document 模型下載/推理/快取管理、Options UI 模型狀態顯示與下載進度條、`TranslationConfig.fallbackType: 'local-onnx'` 配置選項。M2 範圍增列 F-14。先前：F-04 補 M1-48 LLM 直接 fetch 實裝、F-09 補 M1-49 字幕背景樣式三重對比 + 預設/自定義雙模式、F-11 補 M1-50 interceptor arraybuffer 支援 + 渲染日誌降壓。新增 F-12 調試日誌門控（M1-51，八分類中央開關，預設全關，錯誤/降級不受門控）、F-13 字幕翻譯延遲優化（M1-52，分塊翻譯 + 漸進交付 segments-ready/updated + LRU 快取 + 瞬態失敗重試 + 超時覆蓋 body 讀取；**M1-53 兩階段超時**——headers `timeoutMs` 30s + body `BODY_TIMEOUT_MS` 300s，修復本地 LLM 慢生成被 30s 誤殺全片回退原文）；M1 範圍增列 F-12/F-13。先前：F-01 補 YouTube pot token 防護兼容：MAIN world 攔截播放器 timedtext 響應複用；M1-43 捕獲時序修復；M1-45 攔截器 document_start MAIN world 注入 + SPA 換視頻熱重啟 + 跨視頻捕獲失效校驗；M1-46 攔截器重播 lastCapture（1.5s）修復「捕獲早於 bridge 監聽器註冊」競態 + 放寬 timedtext hostname 匹配 + 調試輔助；F-11 範圍擴展：外部接口調用節點診斷全掃描）））））
...
| F-14 | 本地 ONNX 翻譯兜底（M2-24） | 當雲端 LLM API 失敗（網絡錯誤/配額耗盡/離線）時，自動降級至本地 ONNX 小模型進行翻譯，實現完全離線的翻譯兜底。**模型**：`onnx-community/Qwen2.5-0.5B-Instruct`（INT4 ONNX，`dtype: 'q4'` 下載模型倉庫總計約 750MB，含 `onnx/model_q4.onnx` + tokenizer + 配置），平衡翻譯質量與瀏覽器記憶體限制。**架構**：新增 `LocalONNXTranslationProvider` 適配器（實作 `TranslationProvider` 端口），透過 Chrome Message Bus 發送推理請求給 Service Worker，Service Worker 轉發給 Offscreen Document（具備完整 DOM 與 WASM 支援）執行 ONNX Runtime Web 推理。**主引擎支持**：`TranslationConfig.type` 增加 `'local-onnx'`——Options「引擎類型」可選「本地 ONNX 模型（離線）」直接作為主翻譯引擎（Orchestrator primary 選擇 + composition 註冊）；local-onnx 失敗時仍可降級 fallback（mt/none）。**WASM 本地化**：transformers.js v3 默認從 jsdelivr CDN 載入 wasm（CDN 不可達時下載完成後初始化失敗），`copy-static.mjs` 將 `ort-wasm-simd-threaded.{wasm,mjs}` 及 jsep 變體打包進擴充並以 `wasmPaths` 指向本地，`manifest.json` 的 `extension_pages` CSP 含 `'wasm-unsafe-eval'`（MV3 默認 CSP 會攔截 WebAssembly 編譯），推理自包含不依賴外部 CDN。**Options UI**：新增「本地兜底模型」分區，包含：(1) 唯讀模型名稱欄位；(2) 模型狀態標籤（`未下載`/`下載中 xx%`/`已就緒`/`下載失敗`）；(3) 下載進度條（實時顯示位元組下載百分比與速度）；(4) 「下載模型」與「清除快取」按鈕。**模型預加載（補充修復十三）**：新增「預加載模型」按鈕（OMLX 風格手動預載入）——模型已下載時可用，點擊即載入記憶體顯示「已預加載（記憶體）」；同時 Orchestrator 啟動時非阻塞觸發 primary 引擎 `warmup()`（自動觸發式加載）；模型同 offscreen 生命週期只載入一次，跨視頻不需重載；首塊翻譯請求因模型載入慢被 30s 超時誤殺的「字幕卡 71s」問題由 SW 超時提升至 120s + 預熱雙重修復。**降級鏈路**：`TranslationConfig.fallbackType` 新增 `'local-onnx'` 選項，`Orchestrator` 組裝 `TranslationPipeline` 時優先選擇 `local-onnx` 作為 fallback（`local-onnx > mt > undefined`）。若模型尚未下載，拋出錯誤並記錄 `local-onnx-not-downloaded` 診斷，管線繼續降級至 MT 或原文。**清快取一致性（補充修復十五）**：「清除快取 → 重新下載」必須穩定成功——`clearModelCache()` 重置共享載入狀態 + `disposePipeline()` 釋放 ORT wasm 記憶體 + 世代計數（`cacheGeneration`）使載入期間被清快取的陳舊載入作廢（dispose + 不落地 + 拋 `ModelCacheClearedError`）；進度防護保證帶進度的下載不被無進度回調的預熱載入複用；下載被清快取打斷落 `local-onnx-download-stale-load` 診斷（可重試信號）。**流式漸進 + 診斷儀器（補充修復十六）**：`translateStream` 必須真流式——逐 chunk emit 累計全量（首塊數秒 `segments-ready`、後續 `segments-updated`），不得全量跑完才 emit 一次（否則長視頻完全空白）；echo 判定可診斷——`parsedCount` 統計 + echo 診斷內嵌 raw `generated_text` 前 200 字符與耗時，popup「最近失敗」可分辨真回顯 vs 解析誤判；響應 `echoed` 字段透傳、多 chunk 回顯聚合 `local-onnx-echo-chunks`；`local-onnx` 調試日誌分類（第九類）。**零降級行為改變**（用戶指令：僅診斷） | P1 |

| F-15 | 滑動窗口引擎化偏移 + 模型狀態即時顯示（M1-59/M2-27） | **①滑動窗口起點依引擎區分（M1-59）**：M1-57 的滑動窗口統一 `currentTime+2s` 在真實場景暴露兩問題——本地 ONNX 推理慢（每 chunk 5 段 + 每次 sendMessage 往返）需要更大提前量；雲端 LLM/本地 LLM/MT 引擎 +2s 讓 seek 後字幕視覺上「晚 2 秒」。改為**依引擎區分**：本地 ONNX（主引擎 `type === 'local-onnx'` 或兜底 `fallbackType === 'local-onnx'`，配置偵測——`engineId` 在 `TranslationPipeline` 包裝下恆為 `'pipeline'` 不可用）窗口起點 `currentTime+5s`（`WINDOW_START_OFFSET_ONNX_MS=5000`，給慢推理留提前量、優先翻譯播放點前的段）；其餘引擎 `currentTime+0s`（`WINDOW_START_OFFSET_DEFAULT_MS=0`，窗口直接從當前播放點開始）。抽取純函數 `windowStartOffsetMs(translation)`，`getPrioritizedSegments(currentTime, offsetMs)` 參數化偏移。**②Options 模型狀態即時顯示（M2-27）**：Options「本地兜底模型」與「ASR Whisper 模型」分區此前只在頁面生命週期內查一次 `check-status` 且僅 `downloaded` 布爾——無法區分「已下載未載入」與「已預加載到記憶體」，背景預熱完成後 Options 不自動刷新，誤導用戶。改為：offscreen 狀態字段擴充（`local-onnx:status` 增 `loaded/loading/downloading?`、`asr-whisper:status` 增 `downloading?`）；背景預熱成功後 `broadcastToAll` 廣播最新狀態；Options 渲染**五態**（下載中→已預加載（記憶體）→預加載中→已就緒→未下載）並註冊 onMessage 監聽即時刷新、按鈕 disabled 連動 | P1 |
...
| 里程碑 | 範圍 | 說明 |
|---|---|---|
| **M1 — 原生字幕翻譯** | F-01, F-02, F-03, F-04, F-05, F-10, F-11, F-12, F-13, F-15（滑動窗口引擎化偏移） | 一級策略：抓 YouTube 原生字幕 → 翻譯 → 覆蓋層 + 配置界面 + 本地 LLM 服務兼容 + 翻譯失敗診斷可見性 + 調試日誌門控 + 字幕翻譯延遲優化。風險低，優先交付。 |
| **M2 — 實時擷取 ASR** | F-06, F-07, F-14, F-15（模型狀態即時顯示） | 三級策略：tabCapture 音頻 → ASR（雲端/本地可配）→ 翻譯 → 顯示。打通實時管線。新增本地 ONNX 翻譯兜底（F-14），並支持 local-onnx 直接作為主翻譯引擎（離線翻譯）。 |
