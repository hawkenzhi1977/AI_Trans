...
| F-04 | 翻譯引擎配置 | 用戶可選擇雲端 LLM / 本地模型，並填寫端點與密鑰（Options 頁詳配 + Popup 快捷開關；密鑰以獨立安全存儲，不明文入配置） | P0 |
...
| 里程碑 | 範圍 | 說明 |
|---|---|---|
| **M1 — 原生字幕翻譯** | F-01, F-02, F-03, F-04, F-05 | 一級策略：抓 YouTube 原生字幕 → 翻譯 → 覆蓋層 + 配置界面。風險低，優先交付。 |
