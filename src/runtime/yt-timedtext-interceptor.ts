...
function emitTrackInfo(tracklist: YtCaptionTrack[]): void {
  try {
    ...
    // §5.6：emitTrackInfo 是非關鍵路徑（軌道信息發送），不應阻塞關鍵路徑（setOption 驅動字幕載入）。
    // 某些擴展（如 Immersive Translate）可能干擾 postMessage，導致此處拋異常。
    diagLog('interceptor', 'emitTrackInfo failed (non-fatal):', err instanceof Error ? err.message : String(err));
  }
}
...
function ensureCaptionModuleLoaded(): void {
  ...
  // M2-44：入口日誌，記錄每次調用（包括重試），方便排查重試機制是否正常工作。
  diagLog('interceptor', 'ensureCaptionModuleLoaded called, lastCapture videoId:', lastCapture?.videoId ?? 'null');
  try {
    ...
    // M2-44：§5.6 防禦性編程。任何未預料異常不應阻止重試定時器繼續工作，且應留診斷痕跡。
    diagLog('interceptor', 'ensureCaptionModuleLoaded unexpected error:', err instanceof Error ? err.message : String(err));
  }
}
