...
export class NativeCaptionStrategy implements CaptionStrategy {
   ...
   * M2-30：智能選擇最佳字幕軌道。
   * 優先級：
   * 1. ASR 啟用時：音頻語言一致的字幕（人工 > 自動）
   * 2. ASR 未啟用時：英文字幕（人工 > 自動，模糊匹配 en*）
   * 3. 兜底：第一個軌道
   */
  private selectBestTrack(
    tracks: import('../../domain/models/subtitle').CaptionTrack[],
    ctx: StrategyContext
  ): import('../../domain/models/subtitle').CaptionTrack | undefined {
    if (tracks.length === 0) return undefined;

    const asrEnabled = ctx.config.asr.type !== 'none';
    const audioLang = ctx.audioLanguage?.toLowerCase();

    // 輔助函數：模糊匹配語言（en 匹配 en-US、en-GB 等；zh 匹配 zh-Hant、zh-CN 等）
    const langMatches = (trackLang: string, target: string): boolean => {
      const t = trackLang.toLowerCase();
      const tgt = target.toLowerCase();
      return t === tgt || t.startsWith(tgt + '-') || tgt.startsWith(t + '-');
    };

    // 1. 音頻語言一致的軌道（ASR 啟用時優先）
    if (asrEnabled && audioLang) {
      // 優先人工字幕
